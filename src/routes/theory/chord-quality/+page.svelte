<script>
  ...
  function playCurrentChord() {
    tone.init();
    tone.playChord(currentFreqs, true, 0.25);
  }
  ...
  function genQ() {
    ...
    cqShuffle(pool);
    const distractors = pool.slice(0, 3).map(c => c.name);
    const unique = [correctLabel];
    for (const d2 of distractors) { if (!unique.includes(d2)) unique.push(d2); }
    while (unique.length < 4) {
      const rc = CHORD_TYPES[Math.floor(Math.random() * CHORD_TYPES.length)];
      if (!unique.includes(rc.name)) unique.push(rc.name);
    }
    const ch = cqShuffle([...unique]).slice(0, 4);

    return {
      prompt: '<div class="qz-prompt-sub">Name the chord quality</div>\ud83c\udfb5',
      choices: ch,
...
<svelte:head>
  <title>Chord Quality — Ear Training</title>
  <meta name="description" content="Train your ear to identify chord qualities — major, minor, diminished, and more.">
</svelte:head>
...
<div class="nt-wrap">
  <header class="nt-hdr">
    ...
    <nav class="nt-nav">
      ...
      <a href="{base}/theory/interval-recognition" class="pill" style="font-size:13px">Interval Rec.</a>
      <a href="{base}/theory/chord-speller" class="pill" style="font-size:13px">Chord Speller</a>
    </nav>
  </header>
  <div class="nt-stats">
  ...
  <div class="nt-controls">
    ...
    {/if}
    {#if phase === 'active'}
      <button class="nt-btn" onclick={playCurrentChord}>Replay</button>
    {/if}
    {#if showSkip}
